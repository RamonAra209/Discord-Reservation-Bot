...
async def emoji(ctx):
    channel = ctx.channel
    list_of_times = times_between_xy(ceil_dt(dt=datetime(NOW.year, NOW.month, NOW.day, hour=15, minute=30, second=0),
...
async def on_reaction_add(reaction, user):
    ...
    if user.bot == False:
        if reaction.emoji == '\U0000274C': # if X emoji
            ...
            await reaction.message.channel.send(f"Making the reservation!")
            await reaction.message.clear_reactions()
